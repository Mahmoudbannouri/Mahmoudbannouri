<h1 align="center">👋 Hi, I'm Mahmoud Bannouri</h1>
<h3 align="center">🚀 Software Engineering Student | AI & Full-Stack Developer | Future Product Leader</h3>

<p align="center">
  <img src="https://media.giphy.com/media/f3iwJFOVOwuy7K6FFw/giphy.gif" width="220"/>
</p>

---

## 🧠 About Me  

I'm **Mahmoud Bannouri**, a final-year **Software Engineering Student at ESPRIT (Class of 2026)**, passionate about building real, scalable products.  

Currently working as a **Software Developer at Trends Company**, where I design and build professional web applications with:

- **Modern architectures (SOA, Microservices, REST APIs)**
- **AI-assisted workflows**
- **Automation systems (n8n, custom scripts, LLM-powered agents)**  
- **Cloud-based deployments**  

I specialize in integrating **AI + System Architecture** to create smart, scalable solutions. I’ve built assistants and automations using:

- Large Language Models (OpenAI, Gemini, Llama, Ollama)
- Prompt engineering & fine-tuning  
- RAG pipelines  
- End-to-end automation (emails, LinkedIn posts, task management, calendars)  

My goal is to evolve into a **Product Owner / Technical Lead**, combining engineering depth with strategic product thinking.

---

## 🔥 What I Build

### ✔ **AI-Powered Applications**
- Custom assistants (coding, business, productivity)
- Automation pipelines (n8n + custom APIs)
- RAG knowledge systems
- Chatbots for enterprise workflows  
- Text-to-speech / speech-to-text integrations

### ✔ **Full-Stack Products**
- Next.js platforms (Auth, dashboards, API integration)
- Node.js / Express / Prisma backends
- Angular enterprise apps
- Spring Boot microservices
- Real-time apps with WebSockets

### ✔ **Enterprise-Style Architectures**
- SOA & Microservices  
- CI/CD pipelines  
- Dockerized services  
- Authentication / Authorization flows  
- Database design (SQL & NoSQL)

---

## 🧰 Tech Stack

### 👨‍💻 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge)

### 🏗 Frameworks & Architectures
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge)
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge)
![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge)
![Node.js](https://img.shields.io/badge/Node.js-68A063?style=for-the-badge)
![Microservices](https://img.shields.io/badge/Microservices-00BFFF?style=for-the-badge)
![SOA](https://img.shields.io/badge/SOA-00796B?style=for-the-badge)

### 🗄 Databases & Tools
![MySQL](https://img.shields.io/badge/MySQL-00758F?style=for-the-badge)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge)
![Prisma](https://img.shields.io/badge/Prisma-1B222D?style=for-the-badge)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge)

### 🤖 AI & Automation
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge)
![Gemini](https://img.shields.io/badge/Gemini-AF57FF?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-444444?style=for-the-badge)
![n8n](https://img.shields.io/badge/n8n-FF6200?style=for-the-badge)
![ML](https://img.shields.io/badge/Machine_Learning-FF6600?style=for-the-badge)
![DL](https://img.shields.io/badge/Deep_Learning-5D3FD3?style=for-the-badge)
![Generative AI](https://img.shields.io/badge/Generative_AI-6A0DAD?style=for-the-badge)

---

## 🏆 Featured Projects  

### 🏛️ **AI Research Platform (SaaS Edition)**  
A powerful AI-driven **market intelligence** & **idea validation** platform using **Next.js 15, Supabase, Ollama (Llama 3)**, and multi-agent AI flows.

- AI Idea Validator (0–10 scoring with breakdowns)  
- Market, Competitor & Signals Agents  
- Executive Summary Reports  
- Supabase Auth + Modern Dashboard  
- Local AI (Ollama) + Tavily Search + Google Trends  
- Sequential AI orchestration & real-time progress visualization  

---

### 🕌 **UmrahDeals** *(B2B/B2C Umrah management platform)*
- Full travel management system  
- Hotels, flights, packages, visas  
- Multi-role dashboard + supplier portal  
- SOA architecture  
- Real-time data sync  

### 📚 **StudySpark – AI Student Assistant**
- RAG system (summaries, quizzes, flashcards, revision plans)  
- Automatic study optimization  
- Chat-driven learning  

### 👨‍💼 **Best Jobs – Recruitment Platform**
- Job matching engine  
- Candidate filtering  
- Mobile app (Flutter + SQLite)  

### 🧪 **Big Data & ML Projects**
- Bias detection (Kaggle)  
- News political classification  
- Text processing pipelines  

### 🏦 **Vault Mate Bank**
- Web version (Symfony + MySQL)  
- Desktop version (JavaFX)  
- Real banking features + interactive map  

---

## 🏅 Certifications  

- ✔ AI Agents and Agentic AI – 365 Data Science 
- ✔ Hashgraph Developer (Hedera)  
- ✔ Artificial Intelligence Fundamentals  
- ✔ Agile Project Management  
- ✔ Responsive Web Design  
- ✔ Front-End Development Libraries  
- ✔ Java Programming  

---

## 🌐 Languages  
- 🇬🇧 English – Professional  
- 🇫🇷 French – Professional  
- 🇹🇳 Arabic – Native  

---

## 📬 Contact  

📧 Email: **bannourimahmoud67@gmail.com**  
🔗 LinkedIn: **https://www.linkedin.com/in/mahmoudbannouri**  
🌍 Portfolio: **https://mahmoudbannouri.vercel.app**

---
📊 GitHub Stats
<p align="center"> <!-- Main Stats --> <img height="180em" src="https://github-readme-stats.vercel.app/api?username=mahmoudbannouri&show_icons=true&include_all_commits=true&count_private=true&theme=radical"/> <!-- Top Languages --> <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=mahmoudbannouri&layout=compact&langs_count=10&theme=radical"/> </p> <p align="center"> <!-- Streak --> <img src="https://streak-stats.demolab.com?user=mahmoudbannouri&theme=radical" /> </p> <p align="center"> <!-- Trophies --> <img src="https://github-profile-trophy.vercel.app/?username=mahmoudbannouri&theme=radical&no-bg=true&margin-w=15" /> </p>
🐍 Snake Contribution Animation
<p align="center"> <img src="https://github.com/mahmoudbannouri/mahmoudbannouri/blob/output/github-contribution-grid-snake.svg" alt="snake animation" /> </p>
---

## 💬 Final Note  

> 🚀 *I’m always ready to collaborate on challenging and high-impact projects, especially those blending AI, system architecture, and product design.*  
> Let’s build something meaningful.
>
> ---

